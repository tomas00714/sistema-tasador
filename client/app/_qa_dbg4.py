...
async def main():
    async with async_playwright() as pw:
        ...
        r = await page.evaluate(r"""async () => {
            ...
            const rd = await obtenerReportData();
            const sec = document.querySelector('.report-comparables-photos-section');
            return {
                resueltos: comparablesResueltos.length,
                reportDataComps: rd.comparables.map(c => ({id: c.id, photos: (c.photos||[]).length})),
                gruposDOM: sec ? sec.querySelectorAll('.report-comparable-photo-group').length : -1,
                titulos: sec ? [...sec.querySelectorAll('.report-comparable-card-number')].map(t => t.textContent.trim()) : []
            };
        }""")
